...
help_text = {
    ...
    "summaries": "Provides an embed with all the summarries generated by the user",
...
command_args = {
    ...
    "summaries": "!summaries <page number (defaulted to 1)> <option for dm (p, priv, private, dm)",
    "xkcd": "!xkcd",
    "query": "!query <prompt>",
    "login": "!login",
}
    ...
    async def help(self, ctx, command=None):
        ...
        if command:
            ...
            command_embed = discord.Embed(
                ...
                colour=0xB0B0BF,
                description="use '!help <command name>' for more info on any particular command",
            )
            command_embed.set_author(name="UltraChat")
            command_embed.set_thumbnail(url=thumbnail)
            command_embed.set_footer(text="UltraChat by GDSC")
            for command in help_text:
                command_embed.add_field(
                    name=command, value=help_text[command], inline=True
                )
